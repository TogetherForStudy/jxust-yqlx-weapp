...
<template>
  ...
  <view class="min-h-screen bg-page text-fg" :class="[themeStore.rootClass]">
    ...
    <view v-else-if="material" class="pb-20">
      ...
      <view class="bg-surface p-4 mb-2">
        <view class="flex items-start space-x-3">
          ...
          <view class="flex-1 min-w-0">
            <text class="text-fg font-bold text-base line-clamp-2 break-all" :user-select="true">
              {{ material.file_name }}
            </text>
            <view class="flex items-center space-x-3 mt-2 text-xs text-fg-subtle">
        ...
        <view v-if="material.category_path" class="pt-3">
          <text class="text-xs text-fg-muted" :user-select="true">分类：{{ material.category_path }}</text>
        </view>
        ...
        <view v-if="material.tags" class="flex flex-wrap gap-2 mt-3">
          <text
            ...
            :key="tag"
            class="text-xs px-2 py-1 bg-brand-soft text-brand rounded-full"
          >
            {{ tag.trim() }}
          </text>
        </view>
        ...
        <view v-if="material.is_recommended" class="mt-3">
          <view class="inline-flex items-center px-2 py-1 bg-warning-soft rounded-lg">
            <text class="i-lucide-star text-warning w-4 h-4 mr-1"></text>
            <text class="text-xs text-warning font-medium">官方推荐</text>
          </view>
        </view>
      </view>

      <!-- 描述信息 -->
      <view v-if="material.description" class="bg-surface p-4 mb-2">
        <text class="text-fg font-medium text-sm mb-2 block">资料描述</text>
        <text class="text-fg-muted text-sm leading-relaxed" :user-select="true">{{ material.description }}</text>
      </view>

      <!-- 外部链接 -->
      <view v-if="material.external_link" class="bg-surface p-4 mb-2" @tap="openExternalLink">
        <view class="flex items-center justify-between">
          <view class="flex items-center space-x-2">
            <text class="i-lucide-link text-brand w-5 h-5"></text>
            <text class="text-brand text-sm">复制外部链接</text>
          </view>
          <text class="i-lucide-external-link text-brand w-4 h-4"></text>
        </view>
      </view>

      <!-- 统计信息 -->
      <view class="bg-surface p-4 mb-2">
        <text class="text-fg font-medium text-sm mb-3 block">统计信息</text>
        <view class="grid grid-cols-3 gap-4">
          <view class="text-center">
            <text class="text-xl font-bold text-brand block">{{ material.view_count }}</text>
            <text class="text-sm text-fg-subtle mt-1">查看</text>
          </view>
          <view class="text-center">
            <text class="text-xl font-bold text-success block">{{ material.download_count }}</text>
            <text class="text-sm text-fg-subtle mt-1">下载</text>
          </view>
          <view class="text-center">
            <text class="text-xl font-bold text-warning block">{{ material.total_hotness }}</text>
            <text class="text-sm text-fg-subtle mt-1">热度</text>
          </view>
        </view>
      </view>
      ...
      <!-- 评分区域 -->
      <view class="bg-surface p-4 mb-2">
        <text class="text-fg font-medium text-sm mb-3 block">资料评分</text>

        <!-- 平均评分 -->
        <view class="flex items-center justify-center">
          <view class="text-center">
            <text class="text-3xl font-bold text-warning">{{ material.rating ? material.rating.toFixed(1) : '暂无' }}</text>
            <view class="flex items-center justify-center mt-2">
              ...
              >
                <text
                  :class="i <= Math.round(material.rating) ? 'text-warning' : 'text-fg-subtle'"
                  class="i-lucide-star w-4 h-4"
                ></text>
              </view>
            </view>
            <text class="text-xs text-fg-subtle mt-1">{{ material.rating_count }} 人评分</text>
          </view>
        </view>

        <!-- 用户评分 -->
        <view>
          <text class="text-fg-muted text-sm mb-2 block">
          ...
          <view class="flex items-center justify-center space-x-2">
            ...
            >
              <text
                :class="i <= (userRating || material.user_rating || 0) ? 'text-warning' : 'text-fg-subtle'"
                class="i-lucide-star w-6 h-6"
              ></text>
            </view>
          </view>
          <text v-if="userRating || material.user_rating" class="text-center text-sm text-fg-muted mt-2 block">
    ...
    <!-- 底部下载按钮 -->
    <view v-if="material" class="fixed bottom-0 left-0 right-0 bg-surface p-4">
      <view
        class="flex items-center justify-center bg-brand text-white py-3 rounded-xl font-medium w-full"
        @tap="handleDownload"
      >
