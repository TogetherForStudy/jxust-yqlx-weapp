  ...
  <view class="h-screen bg-page flex flex-col text-fg" :class="[themeStore.rootClass]">
    ...
    <view v-if="!currentTeacher && searchKeyword && hasSearched" class="p-4">
      ...
      <view v-if="reviewsLoading" class="text-center py-12">
        <text class="text-fg-muted text-sm">搜索中...</text>
      </view>
      ...
      >
        <view
          ...
          class="bg-surface rounded-lg p-4 shadow-sm active:scale-95 transition-transform duration-200"
        >
          <view class="flex justify-between items-start">
            <view class="flex-1">
              <text class="text-lg font-medium text-fg">{{
                searchResult.teacherName
              }}</text>
              <view class="flex items-center mt-1">
                <text class="text-sm text-fg-muted mr-4"
                  >{{ searchResult.total }} 条评价</text
                >
                <view class="flex items-center">
                  <text
                    class="i-lucide-thumbs-up w-3 h-3 text-success mr-1"
                  ></text>
            ...
            <text class="i-lucide-chevron-right text-fg-subtle w-5 h-5"></text>
          </view>
        </view>
      </view>
      ...
      >
        <text
          class="i-lucide-search-x w-12 h-12 text-fg-subtle mb-4 block mx-auto"
        ></text>
        <text class="text-fg-muted mb-2"
          >未找到教师"{{ searchKeyword }}"的相关评价</text
        >
        <text class="text-sm text-fg-subtle mb-4">请检查教师姓名是否正确</text>
        <view
          @tap="showAddReviewModal"
          class="bg-brand text-white px-4 py-2 rounded-lg inline-block"
        >
          为该教师写评价
        </view>
      </view>
    ...
    <view v-if="!currentTeacher && !searchKeyword" class="p-4">
      <view class="flex flex-col items-center justify-center py-12">
        <text
          class="i-lucide-search w-12 h-12 text-fg-subtle mb-4 block mx-auto"
        ></text>
        <text class="text-fg-muted mb-4">请搜索教师姓名查看评价</text>
        <view
          @tap="showAddReviewModal"
          class="bg-brand text-white px-4 py-2 rounded-lg inline-block"
        >
    ...
    <view v-if="searchKeyword && !hasSearched" class="p-4">
      <view class="flex flex-col items-center justify-center py-12">
        <text
          class="i-lucide-search w-12 h-12 text-fg-subtle mb-4 block mx-auto"
        ></text>
        <text class="text-fg-muted mb-4">请输入教师姓名回车搜索</text>
      </view>
    </view>
    ...
    <view v-if="currentTeacher" class="flex-1 flex flex-col min-h-0">
      <!-- 教师信息头部 -->
      <view class="bg-surface p-4 border-b border-line shrink-0">
        <view class="flex justify-between items-center mb-2">
          <text class="text-lg font-semibold text-fg max-w-[50%] truncate">{{
            currentTeacher
          }}</text>
          <view
            @tap="showAddReviewModal"
            class="bg-brand text-white px-3 py-1 rounded-lg text-sm"
          >
        ...
        <!-- 统计信息 -->
        <view class="flex space-x-6 text-sm text-fg-muted">
          <view class="flex items-center">
            <text class="i-lucide-message-circle w-4 h-4 mr-1"></text>
            <text>{{ teacherReviewsTotal }} 条评价</text>
          </view>
          <view class="flex items-center">
            <text class="i-lucide-thumbs-up w-4 h-4 mr-1 text-success"></text>
      ...
      <view class="flex-1 h-[1px]">
        ...
        >
          <view class="p-4 space-y-4">
            ...
            >
              ...
              <view class="flex justify-between items-start mb-3">
                <view class="flex items-center">
                  <view>
                    <view class="flex items-center">
                      <text class="text-sm text-fg mr-2 max-w-[70%] truncate">{{
                        review.course_name
                      }}</text>
                      <view
                        :class="[
              ...
              <view v-if="review.campus" class="mt-2 flex items-center">
                <text
                  class="i-lucide-map-pin w-3 h-3 text-fg-subtle mr-1"
                ></text>
                <text class="text-xs text-fg-muted">{{ review.campus }}</text>
              </view>
            </view>
            ...
            >
              <text class="text-fg-subtle text-sm">没有更多评价了</text>
            </view>
          </view>
        </scroll-view>
      </view>
      ...
      >
        ...
        <scroll-view scroll-y class="flex-1 min-h-0" style="max-height: 60vh;">
          <view class="p-4 space-y-2">
            ...
            <!-- 课程名称 -->
            <view class="flex flex-col gap-2">
              <text class="text-sm font-medium text-fg">课程名称</text>
              <input
            ...
            <!-- 校区 -->
            <view class="flex flex-col gap-2">
              <text class="text-sm font-medium text-fg">所属校区</text>
              <picker
                :value="campusIndex"
                :range="campusOptions"
            ...
            <!-- 评价态度 -->
            <view class="flex flex-col gap-2">
              <text class="text-sm font-medium text-fg">评价</text>
            ...
            <!-- 评价内容 -->
            <view class="flex flex-col gap-2">
              <text class="text-sm font-medium text-fg">内容</text>
              <textarea
